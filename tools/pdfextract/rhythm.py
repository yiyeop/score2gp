...
def find_stem(head: Glyph, v_lines, tolerance: float = 1.2) -> Stem | None:
    ...
    for x, y0, y1 in v_lines:
        ...
        # 광인들이 98.9% → 95.8%가 됐다. 성부 나누기가 이 방향에 기대고
...
def extract_events(
    ...
    # 길어서 빔이 오선에서 한참 떨어진다 — 실측 달빛소년에서 오선 아래 26pt
    ...
    # 그대로 세면 마디가 넘친다. 실측 ONCE에서 보통 머리는 1.27~1.47칸인데
    ...
    # (아지랑이에서 16분음표 16개가 8개로 뭉쳐 마디가 절반이 됐다.)
...
def _tuplet_marks(
    ...
    바로 아래, 즉 오선 한복판에 놓인다(ONCE의 16분 셋잇단 구간).
